...
- **SubagentStart hook:** tells each sub-agent to orient into the parent session's domain and topic (inherited from the main agent's last `orient` call), and injects a memoryweb dream digest so nested agents don't start cold.
